 ...
 */
export const verifySignature = (idToken: any, options: any) => {
...
const rsaVerifierForKey = (jwk: any): any => {
  const modulus = base64.decodeToHEX(jwk.n);
  const exponent = base64.decodeToHEX(jwk.e);
  return (new RSAVerifier(modulus, exponent) as any);
};

const getJwk = (domain: any, kid: any) => {
